...
const TASK_ICONS = {
    bell: {
        svg: '<path d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>',
    ...
    video: {
        svg: '<path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path>',
        gradient: 'linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%)'
    },
    like: {
        svg: '<path d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"></path>',
        gradient: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)'
    },
    comment: {
        svg: '<path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>',
        gradient: 'linear-gradient(135deg, #10b981 0%, #059669 100%)'
    },
    gaming: {
        svg: '<path d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z"></path>',
        gradient: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)'
    },
    share: {
        svg: '<path d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path>',
    ...
    gift: {
        svg: '<path d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"></path>',
...
function getTaskIconHTML(iconType) {
    ...
    const fill = icon.fill ? 'fill="currentColor"' : 'fill="none" stroke="currentColor"';
    return `
        <div class="task-icon-wrapper" style="background:${icon.gradient}">
            <svg class="task-icon" ${fill} viewBox="0 0 24 24">${icon.svg}</svg>
        </div>
    `;
}

// ===== CARGAR CONFIG DESDE PHP =====
fetch("/config.php")
    .then(res => res.json())
    .then(config => {
        CONFIG = config;

        console.log('%c⚙️ MultiTask Config Loaded', 'color: #a855f7; font-size: 14px; font-weight: bold;');
...
function applyConfiguration() {
    ...
    if (titleElement) {
        titleElement.textContent = CONFIG.mainTitle || '';
    }
            ...
            <div class="task-item" id="${taskId}">
                ...
                <div class="task-content">
                    <div class="task-title">
                        ${task.title}
                        <span class="timer-badge" id="timer${taskNumber}" style="display:none"></span>
                    </div>
...
function updateProgress() {
    ...
    if (unlockBtn) {
        if (done === total && total > 0) {
            ...
            unlockBtn.onclick = () => window.location.href = CONFIG.unlockUrl;
        } else {
